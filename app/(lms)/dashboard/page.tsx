...
import { getStreakDates } from "@/lib/lms/streaks";
...
import { StreakCalendar } from "@/components/lms/streak-calendar";
...
export default async function DashboardPage() {
  ...
  const streakDates = await getStreakDates(supabase, user.id, 30);
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
          <div className="mb-4 flex items-center gap-2">
            <Flame className={`h-4 w-4 ${profile.current_streak > 0 ? "text-orange-400" : "text-[var(--muted-foreground)]"}`} />
          ...
          <div className="mb-4 flex items-center gap-6">
            ...
            <div className="text-center">
              <p className="font-mono text-3xl font-bold text-[var(--muted-foreground)]">
                {profile.longest_streak}
              </p>
              <p className="text-[10px] text-[var(--muted-foreground)]">longest</p>
            </div>
          </div>

          {/* 30-day calendar */}
          <div>
            <p className="mb-2 text-[10px] text-[var(--muted-foreground)]">Last 30 days</p>
